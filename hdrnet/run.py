...
def main(args):
    ...
    with tf.Session(config=config) as sess:
        ...
        for idx, input_path in enumerate(inputs):
            ...
            # -1 means read as is, no conversions.
            im_input = cv2.imread(input_path, -1)
            if im_input.shape[2] == 4:
                log.info(
                    "Input {} has 4 channels, dropping alpha".format(input_path))
                im_input = im_input[:, :, :3]

            # OpenCV reads BGR, convert back to RGB.
...
if __name__ == '__main__':
    ...
    parser.add_argument('checkpoint_dir', default=None,
                        help='path to the saved model variables')
    ...
    # Optional
    parser.add_argument('--lowres_input', default=None,
                        help='path to the lowres, TF inputs')
    parser.add_argument('--hdrp', dest="hdrp", action="store_true",
                        help='special flag for HDR+ to set proper range')
